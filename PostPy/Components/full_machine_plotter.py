...
class PlotMachine:
    ...
    def convergence_history(self, file='stage.log'):
        ...
        names = [r'$max\left| \frac{\partial u_m}{\partial t} \right|$',
                 r'$avr\left| \frac{\partial u_m}{\partial t} \right|$',
                 r'$max\left|\frac{\dot{m}}{\dot{m}_{in}}-1\right|$', 'FLOWRAT']
        for i in range(3):
            if i == 0:
    ...
    def variable_evolution_1D(self, variable, avr_type='massFlowAve'):
        ...
        for r in range(N_rows):
            ...
            if avr_type == 'areaAve':
                avr = [self.machine.rows[r].passage_original.get_area_average(variable, j / (N_j - 1), [0, 1], [0, 1])
                       for j in range(N_j)]
            elif avr_type == 'massFlowAve':
                avr = [
                    self.machine.rows[r].passage_original.get_mass_flow_average(variable, j / (N_j - 1), [0, 1], [0, 1])
                    for j in range(N_j)]
            else:
    ...
    def variable_evolution_2D(self, variable, avr_type='massFlowAve'):
        ...
        for r in range(N_rows):
            ...
            for j in range(N_j):
                var.append(
                    self.machine.rows[r].passage_original.get_pitch_average(variable, j / (N_j - 1), avr_type).tolist())
